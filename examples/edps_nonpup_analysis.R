...
edpsnp <- proc.data(allcounts="ALLCOUNTS_v8.xlsx", age="np", dps="edps") %>%
  filter(n.survey>1, n.nonzero>1)

### Get site information
site.info <- edpsnp %>% select(site, region) %>% distinct()
...
ggplot(data=edps.summ) +
  ...
  xlab('Year') + ylab('Count') + ggtitle("eDPS nonpups")
